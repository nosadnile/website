<template>
    <div>
        <div class="marker-sets">
            <MarkerSet
                v-for="markerSet of filteredMarkerSets"
                :key="markerSet.id"
                :marker-set="markerSet"
                @more="openMore(markerSet)"
            />
        </div>
...
export default {
    ...
    computed: {
        ...
        filteredMarkers() {
            return this.thisMarkerSet.markers
                .filter((marker) => {
                    ...
                    return (
                        marker.type === "player" &&
                        (marker.name.includesCI(this.filter.search) ||
                            marker.playerUuid.includesCI(this.filter.search))
                    );
                })
                .sort((a, b) => {
                    ...
                    if (this.filter.order === "distance") {
                        return (
                            a.position.distanceToSquared(this.controls.position) -
                            b.position.distanceToSquared(this.controls.position)
                        );
                    }
        ...
        filteredMarkerSets() {
            return this.thisMarkerSet.markerSets
                .filter((markerSet) => {
