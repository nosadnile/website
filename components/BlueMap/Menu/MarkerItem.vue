    ...
    <div class="marker-item" :class="{ 'marker-hidden': !marker.visible }">
        <div class="marker-button" :title="marker.id" @click="click(false)">
            <div class="icon" v-if="marker.type === 'player'">
                <img
                    :src="'maps/' + mapId + '/assets/playerheads/' + marker.playerUuid + '.png'"
                    alt="playerhead"
                    @error="steve"
                />
            </div>
            <div class="info">
                ...
                <div class="stats">
                    <div v-if="appState.debug">{{ marker.type }}-marker</div>
                    <div>
                        ({{ position(marker.position.x) }} | {{ position(marker.position.y) }} |
                        {{ position(marker.position.z) }})
                    </div>
                </div>
            </div>
        </div>
        <div
            class="follow-player-button"
            :class="{
                active:
                    controls.controls.followingPlayer &&
                    controls.controls.followingPlayer.id === marker.id,
            }"
            v-if="marker.type === 'player'"
            @click="click(true)"
            :title="$t('markers.followPlayerTitle')"
        >
        ...
        async click(follow) {
            ...
            if (this.marker.type === "player") {
                ...
                if (
                    follow &&
                    cm.controls &&
                    cm.controls.followPlayerMarker &&
                    this.marker.visible
                ) {
